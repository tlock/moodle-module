...
class mod_equella_mod_form extends moodleform_mod {
    ...
    function definition() {
        ...
        $woptions = array(
            EQUELLA_DISPLAY_SAMEWINDOW => get_string('option.pagewindow.same', 'equella'),
            EQUELLA_DISPLAY_POPUP => get_string('option.pagewindow.new', 'equella'),
            EQUELLA_DISPLAY_VIEWER => get_string('option.pagewindow.equellaviewer', 'equella'),
        );
        $mform->addElement('select', 'displaymode', get_string('option.pagewindow', 'equella'), $woptions);
        $mform->setDefault('displaymode', !empty($CFG->resource_popup));
        ...
        foreach( equella_get_window_options() as $option ) {
            ...
            $mform->disabledIf($option, 'displaymode', 'eq', 0);
    ...
    function data_preprocessing(&$defaults) {
        if (!isset($defaults['popup'])) {
            ...
            $defaults['displaymode'] = EQUELLA_DISPLAY_POPUP;
